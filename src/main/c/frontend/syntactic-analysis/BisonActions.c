...
PlayerDef * PlayerDefSemanticAction(int id, int money, int position, TokenLabel strategyToken) {
	...
	// g_parsedPlayers++;  // Commented out - only needed for BoardSim backend
...
DiceDef * DiceDefSemanticAction(int sides) {
	...
	// g_parsedDice++;  // Commented out - only needed for BoardSim backend
...
SimulateBlock * SimulateBlockSemanticAction(int turns) {
	_logSyntacticAnalyzerAction(__FUNCTION__);
	// g_simulateTurns = turns;  // Store the parsed simulate turns globally - only needed for BoardSim backend
