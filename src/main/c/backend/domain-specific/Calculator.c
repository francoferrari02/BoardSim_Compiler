...
ComputationResult executeCalculator(CompilerState * compilerState) {
	...
	// Check if the AST is NULL or not a valid calculator program
	// This happens when the frontend parses BoardSim syntax (Stage II only validates syntax)
	if (program == NULL || program->expression == NULL) {
		logDebugging(_logger, "AST is not a calculator program (likely BoardSim). Returning success for Stage II syntax validation.");
